#!/usr/bin/env python3
"""
Replace Recipes Without Nutritional Data

This script identifies recipes in our database that don't have nutritional data
and replaces them with recipes from the SQLite database that do have nutritional data.
"""
...
class RecipeReplacer:
    ...
    def get_or_create_ingredient(self, ingredient_name, category='other'):
        ...
        if not ingredient:
            # Get the highest existing ingredient ID and increment
            max_id = self.db.query(Ingredient).order_by(Ingredient.id.desc()).first()
            if max_id:
                # Extract number from existing ID and increment
                import re
                match = re.search(r'ingredient_(\d+)', max_id.id)
                if match:
                ...
                next_num = 1
            ingredient_id = f'ingredient_{next_num:06d}'
    ...
    def replace_recipes_without_nutrition(self):
        """Replace recipes without nutritional data"""
        logger.info("🔄 Starting to replace recipes without nutritional data...")
        ...
        logger.info(f"🎉 Completed! Replaced {self.replaced_count} recipes, {self.failed_count} failed")
        return self.replaced_count, self.failed_count
...
def main():
    """Main function to replace recipes without nutritional data"""
    replacer = RecipeReplacer()
    
    try:
        replaced_count, failed_count = replacer.replace_recipes_without_nutrition()
        
        if failed_count == 0:
            logger.info("🎉 SUCCESS! All recipes without nutritional data have been replaced!")
        else:
