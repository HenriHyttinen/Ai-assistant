...
After fixing ingredient quantities, we need to recalculate recipe nutrition
totals from the corrected ingredients.

This script:
1. Goes through all recipes
2. Sums nutrition from all ingredients based on corrected quantities
3. Updates per-serving and total nutrition values
4. Ensures consistency: total = per_serving * servings
"""
...
class RecipeNutritionRecalculator:
    ...
    def recalculate_recipe_nutrition(self, recipe):
        ...
        for ingredient in recipe.ingredients:
            ...
            # Check if ingredient has a valid name (not empty or just punctuation)
            ing_name = (ing_obj.name or '').strip()
            if not ing_name or len(ing_name) < 2:
                # Skip ingredients with empty or invalid names
                continue
            
            # Get quantity in grams (already converted)
            quantity_g = ingredient.quantity or 0
        ...
        old_total_cal = recipe.total_calories or 0
        old_per_serving_cal = recipe.per_serving_calories or 0
        
        # Only update if there's a significant difference (>5%)
        needs_update = False
        if abs(old_total_cal - total_calories) > max(10, old_total_cal * 0.05) or \
           abs(old_per_serving_cal - per_serving_calories) > max(5, old_per_serving_cal * 0.05):
            needs_update = True
        
        if needs_update or old_total_cal == 0:
    ...
    def recalculate_all_recipes(self):
        ...
        logger.info(f"   Recalculated {self.recalculated_count} recipes")
        logger.info(f"   Skipped {self.skipped_count} recipes (no valid ingredients)")
        
        return self.recalculated_count
    
    def verify_calculations(self):
        """Verify that totals = per_serving * servings"""
        logger.info("🔍 Verifying calculations...")
        
        recipes = self.db.query(Recipe).limit(100).all()
        inconsistencies = []
        ...
        for recipe in recipes:
            ...
            # Check calories
            per_serving = recipe.per_serving_calories
            total = recipe.total_calories
            
            if per_serving and total:
                expected_total = per_serving * servings
        ...
        if inconsistencies:
            logger.warning(f"⚠️  Found {len(inconsistencies)} recipes with inconsistencies:")
            for issue in inconsistencies[:5]:
                logger.warning(f"   {issue['recipe'][:50]}: {issue['per_serving']} × {issue['servings']} = {issue['expected_total']}, but total = {issue['total']}")
        else:
            logger.info("✅ All checked recipes have consistent totals")
...
def main():
    ...
    try:
        ...
        recalculator.verify_calculations()
        
        logger.info(f"🎉 SUCCESS!")
        logger.info(f"   Recalculated {recalculated_count} recipes")
